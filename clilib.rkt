...
; logging
; mode 'rotate or 'replace or 'append
(define (get-logger logfile [mode 'replace])
  (case mode
    [(replace) (when (file-exists? logfile) (delete-file logfile))]
    [(rotate) (displayln "Logfile rotation is not supported. Will append instead.")])
  ; returns the following function. newline-mode can be 'newline or 'continue
  (lambda (line [newline-mode 'newline])
    (define (logit p)
      (define printer (if (equal? newline-mode 'newline) displayln display))
      (printer line)
      (printer (uncolor line) p))
    (call-with-output-file logfile logit #:exists 'append)))
